...
float distanceToLine(){
  float dist, up_eq, lo_eq;

  up_eq = abs((desired_position_.y - initial_position_.y) * position_.x
              - (desired_position_.x - initial_position_.x) * position_.y
              + (desired_position_.x * initial_position_.y)
              - (desired_position_.y * initial_position_.x));

  lo_eq = getDistance(initial_position_, desired_position_);
...
bool isOnPendant(){
  ...
  double initial_to_current_distance = getDistance(initial_position_, position_);

  //ROS_INFO("SumIPG: %f | DistIG: %f", initial_to_current_distance+current_to_goal_distance, initial_to_goal_distance);

  return abs((initial_to_current_distance+current_to_goal_distance)-initial_to_goal_distance) < 0.03;
}
...
void bugConditions(){
  ...
  float desired_yaw, err_yaw, dist_position_to_line;
  ...
  if(state_ == GoToPoint){
    ...
    if(!isOnPointRange(position_, hit_point, 0.3)){
      ...
      if(isOnPendant()){
        ...
        float hit_to_goal_distance = getDistance(hit_point, desired_position_);
        if((current_to_goal_distance+0.2 < hit_to_goal_distance)){
          // Bug0 check conditions
          if(abs(err_yaw) < (PI/6) && regions_["front"] > dist_detection+0.1){
            ROS_INFO("Less than 30 deg");
            changeState(GoToPoint);

          } else if(err_yaw > 0 && (abs(err_yaw) > (PI/6)) && (abs(err_yaw) < (PI))
                    && regions_["left"] > dist_detection+0.1){
            ROS_INFO("Between 30 and 180 - to the left");
            changeState(GoToPoint);

          } else if(err_yaw < 0 && (abs(err_yaw) > (PI/6)) && (abs(err_yaw) < (PI))
                    && regions_["right"] > dist_detection+0.1){
            ROS_INFO("Between 30 and 180 - to the right");
            changeState(GoToPoint);
          }
        }
      } /*else{
...
int main(int argc, char **argv)
{
  ...
  ros::ServiceServer service = nh.advertiseService("bugServer/bug2Switch", bugSwitch);
