...
static visualization_msgs::Marker path_start_goal_marker = visualization_msgs::Marker();
...
void publishPathMarkers(){
  if(isPathMarkerReady){
    ...
    //marker_pub.publish(path_start_goal_marker);
...
void initMarkers(){
  ...
  color.g = 0.0;
  color.b = 0.7;
  path_start_goal_marker = createMarker(frame_id, node_name, 9, visualization_msgs::Marker::LINE_STRIP, visualization_msgs::Marker::ADD, geometry_msgs::Point(), scale, color);
  path_start_goal_marker.points.push_back(position_);
  path_start_goal_marker.points.push_back(desired_position_);
  marker_pub.publish(path_start_goal_marker);*/
...
void bugConditions(){
  ...
  if(state_ == GoToPoint){
    ...
    } else if(count_state_time < 5){
      // Wait until the robot is close to boundary
      leaveCondition = false;
      count_tolerance = 0;
